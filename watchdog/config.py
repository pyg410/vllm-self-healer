...
class Config:
    ...
    def validate(self):
        ...
        if not self.vllm_model.strip() or not self.vllm_container_name.strip() or not self.state_file:
            raise ValueError("VLLM_MODEL, VLLM_CONTAINER_NAME and STATE_FILE are required")
        ...
        if self.docker_api_timeout <= self.docker_stop_timeout:
